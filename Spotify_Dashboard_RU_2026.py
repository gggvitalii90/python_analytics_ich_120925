"""
Spotify Songs — Интерактивный дашборд
...
def update_party(genres, years, min_pop):
    ...
    fig = go.Figure(go.Bar(
        ...
        marker=dict(color=party["mean"],
                    ...
                    showscale=True, colorbar=dict(title="Индекс", tickformat=".2f")),
    ...
    fig.update_layout(title="Индекс вечеринки",
...
def update_top_tracks(genres, years, min_pop, top_n, metric):
    ...
    top["label"] = top["track_name"].str[:40] + "  —  " + top["track_artist"].str[:18]
    fig = go.Figure(go.Bar(
